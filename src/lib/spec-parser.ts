import type { OpenAPIObject, PathItemObject, OperationObject, ParameterObject, RequestBodyObject, ResponseObject, MediaTypeObject } from 'openapi-types'
...
function extractSchema(mediaType: MediaTypeObject | undefined): Record<string, unknown> | undefined {
...
function parseRequestBody(requestBody: RequestBodyObject | undefined): ParsedRequestBody | undefined {
...
function parseResponses(responses: OperationObject['responses']): ParsedResponse[] {
  if (!responses) return []
...
export function parseSpec(spec: OpenAPIObject): ParsedEndpoint[] {
  ...
  for (const [path, pathItem] of Object.entries(paths)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      endpoints.push({
        ...
        requestBody: parseRequestBody(operation.requestBody as RequestBodyObject | undefined),
